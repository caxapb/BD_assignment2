...
def test_spark_connection():
    ...
    for attempt in range(max_retries):
        try:
            ...
            print("\nAttempting to verify table existence through Spark...")
            
            # Test read with explicit case sensitivity handling
            stats_df = spark.read \
                .format("org.apache.spark.sql.cassandra") \
                .options(table="stats", keyspace="search_engine") \
                .load()
            
            print("\nSuccessfully read stats table:")
            stats_df.show(1, truncate=False)
